...
def relitu(siji, target, jijie1num, jijie2num, index, xp ,yp):
    juzhen = get_juzhen_from_jijie(siji[jijie1num], siji[jijie2num])
    data_1 = {}
    for i in range(len(index[jijie1num])):
        data_1[index[jijie1num][i]] = juzhen[:, i]
    pd_data = pd.DataFrame(data_1, index=index[jijie2num], columns=index[jijie1num])
    ax = sns.heatmap(pd_data, annot=True, fmt='.3f',vmin = 10000 , vmax = 60000)
...
if __name__ == "__main__":
    ...
    plt.hist(r,bins, alpha=0.5,label= 'r',color="#539caf")
    plt.hist(g,bins, alpha=0.5,label= 'g',color="#7663b0")
    plt.hist(b,bins, alpha=0.5,label= 'b')

    plt.show()
    plt.close()'''
    """a= np.array([1,1,1]) /1.0
    b = np.array([1,1,2]) * 2 /1.0
    x = torch.tensor(a)
    y = torch.tensor(b)
    print(a)
    print(b)
    ...
    KL = F.kl_div(logp_x, p_y, reduction='sum')
    print(KL)"""
    #get_fenbu(r'C:\Users\owl\Desktop\tifftest\beijing_2019\20190310\beijing_2019_20190310_543.tif')
    """x = np.random.random((1,10))
    y = np.random.random((1,10))
    print(x)
    print(y)
    print(get_KL_from_fenbu(x,y))"""
